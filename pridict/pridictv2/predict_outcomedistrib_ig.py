    ...
    def _get_integrated_gradients(self, models, dloader, bg_set, m_steps=50, y_ref=[]):
        ...
        for indx_batch, sbatch in tqdm(enumerate(dloader)):
            ...
            with torch.set_grad_enabled(requires_grad):
                ...
                y_hat_logit = torch.exp(y_hat_logit)[:,outcome_indx:outcome_indx+1]
                ...
                # f_xbase = y_hat_logit.reshape(bsize, m_steps, -1)[:,:,:].sum(axis=1)
                ...
                init_grad_s = init_grad*dist_init
                mut_grad_s = mut_grad*dist_mut
                seqfeat_grad_s = seqfeat_grad*dist_seqlevelfeat
                ...
                #     seqfeat_grad_s = seqfeat_grad*dist_seqlevelfeat
                # else:
                #     init_grad_s = init_grad
                #     mut_grad_s = mut_grad
                #     seqfeat_grad_s = seqfeat_grad

                ig_init_grad_scaled = ig_explainer.compute_riemann_trapezoidal_approximation(init_grad_s)
                ig_mut_grad_scaled = ig_explainer.compute_riemann_trapezoidal_approximation(mut_grad_s)
                # [num_baselines, seqlevelfeat_dim]
                ig_seqfeat_grad_scaled = ig_explainer.compute_riemann_trapezoidal_approximation(seqfeat_grad_s)
                ...
                # print('fsum:', fsum_)
                # print('(sum_attr_-fsum_).abs():',(sum_attr_-fsum_).abs())
                diff = (fsum_ - sum_attr_).abs()/fsum_.abs().clip(min=1e-6) # (delta_prediction - attribution_sum)/delta_prediction
                # print('diff abs:', diff)
